...
bee6_wfo.py
============
Walk-forward optimization for the first BEE6 WaveTrend strategy.
"""
...
def walk_forward_optimization(
    ...
    while start + opt_bars + live_bars <= n:
        ...
        for (
            ...
            strat = Bee6Strategy(params, fee_rate=fee_rate)
        ...
        strat = Bee6Strategy(best_params, fee_rate=fee_rate)
        (
            trades_live,
            equity_live,
            final_cap_live,
        ...
        if not trades_live.empty:
            ...
            trades_live["wt_long_breakeven_trigger_pct"] = best_params.get(
                "wt_long_breakeven_trigger_pct",
                WT_LONG_BREAKEVEN_TRIGGER_PCT,
            )
        ...
        window_stats.append(
            {
                ...
                "best_wt_long_breakeven_trigger_pct": best_params.get(
                    "wt_long_breakeven_trigger_pct",
                    WT_LONG_BREAKEVEN_TRIGGER_PCT,
                ),
...
def get_latest_best_params(windows_df: pd.DataFrame) -> dict:
    ...
    long_breakeven_trigger_pct = (
        float(recent["best_wt_long_breakeven_trigger_pct"].mode().iloc[0])
