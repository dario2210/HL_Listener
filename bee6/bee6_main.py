...
bee6_main.py
=============
Entry point for the first BEE6 WaveTrend strategy.
"""
...
from bee6_stats import compute_stats, fee_summary_by_period, print_wfo_windows
...
def run_backtest(
    ...
    if save:
        trades.to_csv("bee6_backtest_trades.csv", index=False)
        equity.to_csv("bee6_backtest_equity.csv", index=False)
        print("\n[SAVE] bee6_backtest_trades.csv, bee6_backtest_equity.csv")
...
def run_wfo(
    ...
    if save:
        if all_trades is not None and not all_trades.empty:
            all_trades.to_csv("bee6_wfo_trades.csv", index=False)
        if equity_wfo is not None:
            equity_wfo.to_csv("bee6_wfo_equity.csv", index=False)
        if windows_df is not None and not windows_df.empty:
            windows_df.to_csv("bee6_wfo_windows.csv", index=False)
        print("[SAVE] bee6_wfo_trades.csv, bee6_wfo_equity.csv, bee6_wfo_windows.csv")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="BEE6 WaveTrend bot")
    ...
    parser.add_argument("--interval", default=BINANCE_INTERVAL, choices=["1h"], help="Timeframe fixed to 1h")
