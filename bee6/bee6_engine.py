...
bee6_engine.py
===============
Shared decision layer for the first BEE6 WaveTrend strategy.
"""
...
from bee6_params import SHORT_TRADING_ENABLED, WT_ZERO_LINE
...
def _long_next_candle_entry_ok(bar: BarData, prev_bar: BarData) -> bool:
    if _flag_value(prev_bar.wt_green_dot):
        return True
    return _float_or_nan(bar.bars_since_wt_green_dot) == 1.0
...
def generate_entry_signal(
    ...
    """
    Entry logic for BEE6:
      - long-only
      - H1 green dot must be on the previous closed candle
      - both H1 WT lines on that setup candle must be at or below the tested entry level
    """
    ...
    setup_level = _signal_level(prev_bar)
    ...
    long_cond = (
        allow_longs
        and _long_next_candle_entry_ok(bar, prev_bar)
        and prev_bar.wt1 <= long_entry_max_above_zero
        and prev_bar.wt2 <= long_entry_max_above_zero
        and setup_level >= min_level
    )
    ...
    meta = {
        ...
        "entry_signal_level": round(setup_level, 4),
        "prev_wt1": round(prev_bar.wt1, 4),
        "prev_wt2": round(prev_bar.wt2, 4),
        "setup_wt1": round(prev_bar.wt1, 4),
        "setup_wt2": round(prev_bar.wt2, 4),
        "setup_delta": round(prev_bar.wt_delta, 4),
    ...
    if long_cond:
        return Signal(
            action="open_long",
            reason="WT_H1_GREEN_DOT_NEXT_CANDLE_LONG",
            meta={
                **meta,
                "cross_type": "bullish",
                "bars_since_green_dot": 1.0,
                "long_entry_level_h1": long_entry_max_above_zero,
            },
        )
    return Signal(action="none")
...
def generate_partial_exit_signal(
    ...
    """BEE6 does not scale out; profit management only arms break-even."""
    if bool(params.get("wt_disable_partial_exits", True)):
        return Signal(action="none")
...
def generate_exit_signal(
    ...
    """
    Exit logic for BEE6:
      - +1% profit arms a break-even stop for the full long position
      - a later touch of entry price exits at break-even
      - an H1 red dot exits when both WT lines are at or above the tested close level
    """
    ...
    if position.side == "long":
        ...
        if (
            ...
            breakeven_reason = "LONG_BREAKEVEN_STOP"
        ...
        if _cross_down(bar, prev_bar):
            ...
            if not _long_close_level_ok(bar, params):
                return Signal(action="none")
            meta = _meta("WT_H1_RED_DOT_LEVEL_EXIT_LONG")
            meta["h1_red_close_count"] = position.h1_red_close_count
            meta["long_close_level_h1"] = float(
                params.get("wt_long_close_min_level", params.get("wt_long_exit_min_level", 0.0))
            )
            meta["exit_red_dot"] = True
            return Signal(
                action="close_force",
                reason="WT_H1_RED_DOT_LEVEL_EXIT_LONG",
                meta=meta,
            )
