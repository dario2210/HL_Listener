"""
bee6_dashboard.py  -  BEE6 WaveTrend Dashboard  http://IP:8071
"""
...
from bee6_wfo      import get_latest_best_params, walk_forward_optimization
from bee6_stats    import (
    compute_stats, fee_summary_by_period,
    breakdown_by_side, breakdown_by_period, breakdown_by_cross_type,
)
from bee6_binance  import update_csv_cache, wfo_bars
...
def _result_filename(result_data: dict, purpose: str, ext: str, stamp: str | None = None) -> str:
    ...
    return f"bee6_{mode}_{symbol}_{tf}_{period}_{window}_{_safe_slug(purpose)}_{stamp}.{ext}"
...
def _metadata_comment_block(result_data: dict, prefix="# ") -> str:
    lines = [f"{prefix}BEE6 export metadata"]
...
def _save_result_file(result_data: dict) -> str:
    ...
    payload = {
        "schema": "bee6_dashboard_result_v1",
...
def _direction_flags(direction) -> tuple[str, bool, bool]:
    # BEE6 currently tests only the long side. Short signals are ignored by design.
    ...
    params.update(
        {
            ...
            "wt_long_tp1_pct": breakeven_trigger_pct,
            ...
            "wt_short_tp1_pct": breakeven_trigger_pct,
            "wt_short_tp1_fraction": 0.0,
...
def _grid_overrides_from_controls(
    ...
    return {
        ...
        "wt_h4_long_filter_max": [float(DEFAULT_PARAMS["wt_h4_long_filter_max"])],
        "wt_h4_long_close_min": [float(DEFAULT_PARAMS["wt_h4_long_close_min"])],
        "wt_long_emergency_sl_capital_pct": [0.0],
...
PARAM_SUMMARY_LABELS = {
    ...
    "wt_long_entry_window_bars": "Entry delay H1",
...
def hero_banner() -> html.Div:
    return html.Div([
        html.Div([
            ...
            html.H2("BEE6 WaveTrend long H1"),
            html.P(
                "BEE6 testuje tylko long na H1: wejscie nastepna swieca po zielonej kropce "
                "w niskiej strefie WT1/WT2, a wyjscie na czerwonej kropce w wysokiej strefie."
            ),
        ], className="hero-copy"),
        html.Div([
...
def fig_report_price_wt(result_data: dict) -> go.Figure:
    ...
    fig.update_layout(
        **PT,
        height=720,
        title=f"{symbol.upper()} {tf} | Cena + WaveTrend H1",
        showlegend=True,
...
def _build_report_pdf(result_data: dict) -> bytes:
    ...
    story = [
        Paragraph("BEE6 - raport wynikow", styles["TitleBee"]),
        Paragraph(
    ...
    try:
        ...
        story.extend(_figure_flowable("Cena + WT H1", fig_report_price_wt(result_data), styles, width=1120, height=700))
        if not fee_df.empty:
...
def _trade_detail_panel(trade: dict | None) -> html.Div:
    ...
    return html.Div([
        ...
        html.Div([
            ...
            row2("Cross", fmt_text(trade.get("entry_cross_type")),
                 "Window WFO", fmt_text(trade.get("window_id", "n/d"))),
            row2("Strefa", fmt_text(trade.get("entry_zone")),
                 "Setup level", fmt_float(trade.get("entry_signal_level"), ".2f")),
            html.Div(style={"borderTop": f"1px solid {C['border']}", "margin": "8px 0"}),
            ...
            row2("Trigger", fmt_text(trade.get("exit_trigger", trade.get("reason"))),
                 "Poziom sygnalu", fmt_float(trade.get("exit_signal_level"), ".2f")),
        ]),
...
def _build_pine_trades_overlay(result_data: dict) -> str:
    ...
    return f"""//@version=5
{metadata}
indicator("BEE6 Trades Overlay with Trade Numbers", overlay=true, max_labels_count=500)

// Generated by BEE6 dashboard: {generated}
...
def _diagnostic_signal_overlays(
    ...
    for pos, (_, row) in enumerate(df.iterrows()):
        ...
        if side == "long" and accepted_trade is None and pos + 1 < len(df):
            next_ts = pd.to_datetime(df.iloc[pos + 1]["time"], utc=True, errors="coerce")
            if not pd.isna(next_ts):
                accepted_trade = trade_lookup.get((_unix_seconds(next_ts), side))
...
def sidebar():
    ...
    return html.Div([
        # nagłówek
        html.Div([
            html.Div("BEE6 Bot",style={"fontSize":"16px","fontWeight":"700","color":C["text"]}),
            html.Div("WaveTrend long H1 profile",style={"fontSize":"11px","color":C["muted"],"marginTop":"2px"}),
        ...
        html.Div([
            ...
            html.Div([
                html.Div([field("Long entry level H1", inp("inp-bt-long-zone", BT_DEFAULT_LONG_OPEN_H1, type="number", step=1))], style={"flex":"1"}),
                html.Div([field("Red-dot exit level H1", inp("inp-bt-long-close-level", BT_DEFAULT_LONG_CLOSE_H1, type="number", step=1))], style={"flex":"1"}),
...
app = dash.Dash(
    __name__,
    title="BEE6 WaveTrend Dashboard",
...
app.clientside_callback(
    ...
    function(chartPayload, activeTab) {
        ...
        if (window.dash_clientside && window.dash_clientside.bee6_dashboard) {
            return window.dash_clientside.bee6_dashboard.render(chartPayload);
        }
        return "";
    }
    """,
...
def _worker(
    ...
    try:
        ...
        if run_mode == "backtest":
            ...
            strat = Bee6Strategy(strategy_params, fee_rate=fee_rate_val)
...
def _build_chart_tab(
    ...
    return html.Div([
        ...
        html.Div([
            ...
            html.Div([
                _legend_chip("pełny punkt", C["green"], "realne wejście long na wykresie ceny"),
                _legend_chip("żółty", C["amber"], "odrzucone przez poziom sygnału"),
            ...
            html.Div(
                ...
                "wysokość i pokazuje WT1/WT2 dla H1. W trybie Diagnostyka kropki H1 pokazują również "
        ...
        if tab == "equity":
            ...
            content = html.Div([
                ...
                html.Div([
                    ...
                    html.Div(
                        "Górny panel pokazuje cenę, linię EMA filtra i markery trade'ów. Dolny panel ma tę samą "
                        "wysokość i pokazuje WT1/WT2 dla H1 z zielonymi i czerwonymi kropkami przecięć.",
                        className="chart-caption",
                    ),
                ], className="result-panel"),
                html.Div(
...
if __name__ == "__main__":
    ...
    a = p.parse_args()
    print(f"\n{'='*50}\n  BEE6 WaveTrend Dashboard  ->  http://{a.host}:{a.port}\n{'='*50}\n")
