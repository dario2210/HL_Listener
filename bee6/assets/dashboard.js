(function () {
  ...
  function ensureChart(container) {
    const current = window.__BEE6LightweightChart;
    ...
    window.__BEE6LightweightChart = buildChartState(container);
    return window.__BEE6LightweightChart;
  }

  function showRenderError(container, error) {
    console.error("BEE6 chart render failed", error);
    destroyChartState(window.__BEE6LightweightChart);
    window.__BEE6LightweightChart = null;
    container.innerHTML = [
