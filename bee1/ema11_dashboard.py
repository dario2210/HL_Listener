...
def hero_banner() -> html.Div:
    return html.Div([
        html.Div([
            ...
            html.H2("EMA-11 strategy console"),
            html.P(
                "Widok wynikow zostaje po stronie bee1, ale warstwa prezentacji "
                "jest ustawiona pod styl bee3 i wykres Lightweight Charts."
            ),
        ], className="hero-copy"),
        html.Div([
            html.Div("Note", className="hero-note-title"),
            html.P(
                "WFO dalej liczy te same dane i te same metryki. Zmieniamy tylko "
                "to, jak sa pokazywane po uruchomieniu symulacji."
            ),
        ], className="hero-note"),
...
def fig_pdist(wd):
    if wd is None or wd.empty: return go.Figure(layout=PT)
    fig=make_subplots(rows=2,cols=2,subplot_titles=["TP","SL","Trail","EMA"],
                      vertical_spacing=0.15,horizontal_spacing=0.1)
    for (col,r,c),clr in zip([("best_tp",1,1),("best_sl",1,2),
                               ("best_trail",2,1),("best_ema_len",2,2)],
...
def lightweight_chart_payload(
    ...
    df = _chart_source_df(symbol, tf)
    if df is None or df.empty:
        return {"candles": [], "lines": [], "markers": [], "focusRange": None}
    ...
    line_specs = [
        ("ema_200", "EMA 200", C["amber"], 2.4, True),
        ("ema_150", "EMA 150", C["purple"], 1.6, False),
        ("ema_100", "EMA 100", C["blue"], 1.4, False),
        ("ema_50", "EMA 50", C["muted"], 1.2, False),
    ]
    lines = []
    ...
    filtered_trades = _filter_chart_trades(trades_df, filter_mode)
    ...
    return html.Div([
        ...
        html.Div([
            ...
            dcc.Checklist(id="chk-tp",
                options=[{"label":f" {v*100:.1f}%","value":v}
                         for v in [0.025,0.035,0.040,0.050,0.060,0.075]],
            ...
            dcc.Checklist(id="chk-sl",
                options=[{"label":f" {v*100:.2f}%","value":v}
                         for v in [0.003,0.005,0.0075,0.010,0.0125,0.015]],
                value=SL_GRID, inline=True,
                inputStyle={"marginRight":"4px","accentColor":C["blue"]},
                labelStyle={"color":"#e8eaf6","fontSize":"12px","marginRight":"10px"}),
            sec("Siatka Trail"),
            dcc.Checklist(id="chk-trail",
                options=[{"label":f" {v}","value":v} for v in [2,3,4,5,6,8]],
                value=TRAIL_DROP_GRID, inline=True,
                inputStyle={"marginRight":"4px","accentColor":C["blue"]},
                labelStyle={"color":"#e8eaf6","fontSize":"12px","marginRight":"10px"}),
            sec("Siatka EMA"),
            dcc.Checklist(id="chk-ema",
                options=[{"label":f" {v}","value":v} for v in [50,100,150,200]],
...
.wt-drp .Select-placeholder { color: """ + C["muted"] + """ !important; font-size:13px !important; }
.wt-drp .Select-value        { color: """ + C["text"] + """ !important; }
.wt-drp .Select-value-label  { color: """ + C["text"] + """ !important; font-size:13px !important; }
.wt-drp .Select-input input  { color: """ + C["text"] + """ !important; }
.wt-drp .Select-arrow        { border-top-color: """ + C["muted"] + """ !important; }
...
.wt-drp [class*="control"]       { background:""" + C["surf2"] + """ !important; border-color:""" + C["border"] + """ !important; min-height:42px !important; box-shadow:none !important; }
.wt-drp [class*="singleValue"]   { color:""" + C["text"] + """ !important; font-size:13px !important; }
.wt-drp [class*="placeholder"]   { color:""" + C["muted"] + """ !important; font-size:13px !important; }
...
.wt-drp [class*="dropdownIndicator"] svg { fill:""" + C["muted"] + """ !important; }
.wt-drp [class*="indicatorSeparator"] { background:""" + C["border"] + """ !important; }
.wt-drp [class*="Input"] input   { color:""" + C["text"] + """ !important; }
...
def _worker(symbol, tf, market, start, end, capital,
    ...
    try:
        ...
        saved = (ep.TP_GRID, ep.SL_GRID, ep.TRAIL_DROP_GRID,
                 ep.EMA_LEN_GRID, ep.OPT_DAYS, ep.LIVE_DAYS, ep.INITIAL_CAPITAL)
        ep.TP_GRID         = tp_grid    or TP_GRID
        ep.SL_GRID         = sl_grid    or SL_GRID
        ep.TRAIL_DROP_GRID = trail_grid or TRAIL_DROP_GRID
        ep.EMA_LEN_GRID    = ema_grid   or EMA_LEN_GRID
        ...
        (ep.TP_GRID, ep.SL_GRID, ep.TRAIL_DROP_GRID,
         ep.EMA_LEN_GRID, ep.OPT_DAYS, ep.LIVE_DAYS, ep.INITIAL_CAPITAL) = saved
...
@app.callback(
    ...
    State("store-chart-filter","data"),
    State("store-selected-trade","data"),
    prevent_initial_call=True,
)
def poll(_, tab, prev_result, chart_filter_val, selected_trade_val):
    g = gs()
    status   = g["status"]
    progress = g["progress"]
    result   = g["result"]
    ...
    result_out = result if result is not None else dash.no_update
    ...
    if not r:
        ...
        return status, progress, result_out, None, [], empty
    ...
    return status, progress, result_out, chart_payload, metrics, content
